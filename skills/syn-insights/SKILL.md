...
```bash
syn costs summary                              # platform-wide summary
syn costs session <session-id>         # cost breakdown for one session
...
syn observe tools <session-id> --limit 200
```
...
1. `syn costs execution <id>` to find which phase was most expensive
2. `syn costs session <session-id>` for the expensive phase: check `cost_by_model` and `cost_by_tool`
3. `syn observe tokens <session-id>` to check cache hit ratio

**"The workflow produced no artifacts. What did the agent do?"**
1. `syn sessions list --workflow <id>` to find the session IDs
2. `syn observe tools <session-id>` to trace every tool call; look for errors or blocked tools

**"I want to see all sessions from the last run."**
`syn sessions list --workflow <workflow-id>` lists all sessions in order, most recent first.
